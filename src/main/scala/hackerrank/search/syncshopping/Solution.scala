...
package hackerrank.search.syncshopping
...
object Solution {
  ...
  */
  ...
  def buildDistanceMatrix(n: Int, grid: Grid, startNode: Int, shoppingCenters: Array[FishSet]): Array[(Int, FishSet)] = {
    val matrix = Array.fill[(Int, FishSet)](n)((Integer.MAX_VALUE, FishSet(0, 0)))
    matrix(startNode) = (0, shoppingCenters(startNode))
    var frontier = Set(startNode)
    while (frontier.nonEmpty) {
      val newFrontier = for {
        node <- frontier
        (distance, fish) = matrix(node)
        (neighbour, edge) <- grid(node)
        newDistance = distance + edge
        if newDistance < matrix(neighbour)._1
      } yield {
        matrix(neighbour) = (newDistance, fish ++ shoppingCenters(neighbour))
        neighbour
      }
  ...
  def solve(task: Task, distanceMatrix: Array[Array[(Int, FishSet)]], fishMap: Map[Int, IndexedSeq[Int]]): Path = {
    ...
    def queuePath(path: Path): Unit = {
      path.bigCatEstimate = distanceMatrix(path.bigCat.lastNode)(task.N - 1)._1
      path.littleCatEstimate = distanceMatrix(path.littleCat.lastNode)(task.N - 1)._1
      ...
      if (path.score < finalResult.maxTime) {
        if (isFinalCondition(path)) {
          ...
          if (missingFish.isEmpty) {
            queuePath(Path(
              path.bigCat.movingTo(task.N - 1, distanceMatrix(path.bigCat.lastNode)(task.N - 1)._1),
              path.littleCat.movingTo(task.N - 1, distanceMatrix(path.littleCat.lastNode)(task.N - 1)._1),
              path.bigFish,
              path.littleFish,
              1
            ))
            ...
            if (path.level == 0) {
              for (node <- targetNodes) {
                val (distance, fish) = distanceMatrix(path.bigCat.lastNode)(node)
                queuePath(Path(
                  path.bigCat.movingTo(node, distance),
                  path.littleCat,
                  path.bigFish ++ fish,
                  path.littleFish,
                  1))
              ...
              for (node <- targetNodes) {
                val (distance, fish) = distanceMatrix(path.littleCat.lastNode)(node)
                queuePath(Path(
                  path.bigCat,
                  path.littleCat.movingTo(node, distance),
                  path.bigFish,
                  path.littleFish ++ fish,
                  0))
              }
            }
          }
  ...
  def main(args: Array[String]): Unit = {
    ...
    val forwardDistances = buildDistanceMatrix(task.N, task.grid, task.N - 1, task.shoppingCenters)
    val backwardDistances = buildDistanceMatrix(task.N, task.grid, 0, task.shoppingCenters)

    val distanceMatrix = (0 until task.N).toArray.map(i => buildDistanceMatrix(task.N, task.grid, i, task.shoppingCenters))
